  ...
  return (
    <div className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border-l-4 ${
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className={`font-semibold ${getTextClass('default')} truncate`}>
                {grupo.name}
              </h3>
              {!grupo.is_active && (
                <span className={`px-2 py-1 ${colors.extra.bg} ${getTextClass('muted')} text-xs rounded-full`}>
                  Inactivo
                </span>
              )}
              {isExpired && (
            ...
            <p className={`text-sm ${getTextClass('muted')} mb-2`}>
          ...
          <div className="flex gap-1">
            <button
              ...
              className={`p-1 ${getTextClass('light')} hover:${colors.primary.text} transition-colors`}
              title="Ver detalles"
            >
        ...
        {/* Description */}
        {grupo.description && (
          <p className={`text-sm ${getTextClass('muted')} mb-3 line-clamp-2`}>
            {grupo.description}
          </p>
        )}
        ...
        <div className="grid grid-cols-3 gap-4 mb-3 text-center">
          <div>
            <div className={`text-lg font-semibold ${getTextClass('default')}`}>
              {grupo.member_count || 0}
            </div>
            <div className={`text-xs ${getTextClass('light')}`}>Miembros</div>
          </div>
          <div>
            <div className={`text-lg font-semibold ${getTextClass('default')}`}>
              {grupo.default_currency} {grupo.total_expenses?.toLocaleString() || 0}
            </div>
            <div className={`text-xs ${getTextClass('light')}`}>Gastos</div>
          </div>
          <div>
            <div className={`text-lg font-semibold ${getTextClass('default')}`}>
              {grupo.default_currency}
            </div>
            <div className={`text-xs ${getTextClass('light')}`}>Moneda</div>
          </div>
        </div>

        {/* Date Range */}
        {(grupo.start_date || grupo.end_date) && (
          <div className={`flex items-center gap-2 text-xs ${getTextClass('light')} mb-2`}>
        ...
        <div className="flex items-center justify-between text-xs">
          <div className={`flex items-center gap-3 ${getTextClass('light')}`}>
